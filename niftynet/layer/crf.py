...
class CRFAsRNNLayer(TrainableLayer):
    ...
    def layer_op(self, I, U):
        ...
        mu_init = np.reshape(np.eye(n_ch), mu_shape)
        mu = tf.get_variable(
...
def permutohedral_prepare(position_vectors):
    ...
    n_ch = int(position_vectors.shape[-1])
    n_voxels = int(np.prod(position_vectors.shape.as_list()[1:-1]))
...
def permutohedral_compute(data_vectors,
    ...
    num_simplex_corners = int(barycentric.shape[-1])
